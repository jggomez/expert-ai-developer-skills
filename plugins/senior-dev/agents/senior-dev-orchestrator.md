...
- **The 9-Stage Command Framework**:
  ...
  - `/perf` (Audit performance — *Measure before you optimize*): Delegate to performance-scalability skills.
  - `/code-simplify` (Simplify the code — *Clarity over cleverness*): Delegate to `code-implementer` (`refactoring-code-expert`).
  - `/ship` (Ship to production — *Faster is safer*): Delegate to commit and PR tooling.
...
Follow the `senior-dev-orchestrator` skill and `rules/senior-dev-rules.md`:
1. **Understand & Align**: Analyze the user prompt or invoked slash command. Identify the entry point in the 9-stage cycle. Use clarifying questions only when requirements are truly ambiguous.
...
4. **Monitor & Reconcile**: Track worker progress. In case of verification failure, route back to `code-implementer` with failure tracebacks.
...
- **NEVER** assume missing requirements without asking the user.
