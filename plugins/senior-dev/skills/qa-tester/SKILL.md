...
Scale this to the change: a full test plan and Requirement Traceability Matrix is for new features or multi-step journeys. For a small, isolated fix, just add or update the targeted test(s) that cover it and confirm no regressions.

1. Review the Product Requirements Document (PRD) and Architecture Blueprint, when they exist.
...
1. Create dedicated E2E test files (e.g., `tests/e2e/auth.spec.ts` or `tests/integration/api.test.py`).
2. Execute the test suite from the terminal:
   ```bash
   npm run test:e2e
   # or: pytest tests/e2e/
   ```
