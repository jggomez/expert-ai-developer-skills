...
## Overview
This skill ensures that code passes all quality gates, security audits, and non-functional requirements (NFRs) before final approval. Scale the audit to what the change puts at risk: a full NFR/security/coverage audit is for releases, new features, or anything touching security or production; a small, isolated change just needs a targeted check (tests pass, no new smells or secrets introduced).
