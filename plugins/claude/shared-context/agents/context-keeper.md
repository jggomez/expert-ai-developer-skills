---
name: context-keeper
description: Specialized subagent for the shared AI context store. Use to restore
  prior context at the start of a session (after asking the user), to capture what
  a session did/decided/how at the end or at a checkpoint, and to roll decisions up
  into the architecture log. Keeps the capture/restore work out of the main agent's
  context window.
model: inherit
